...
<body>
				...
				<div class="content-wrapper"
					...
					<table
						...
						<tr>
							...
							<th>Fullname</th>
							<th>Email</th>
							<th>Total buy <svg xmlns="http://www.w3.org/2000/svg" width="16"
  ...
  <path fill-rule="evenodd"
						...
						<php:forEach items="${userTops }" var="userTop" varStatus="i">
							<php:forEach items="${users }" var="user">
								<php:if test="${userTop.getId() == user.getId() }">
									<tr style="font-weight: 700; font-family: Verdana, sans-serif;">
										<td>${i.index+1 }</td>
										<td><img style="width: 60px; height: 60px"
											class="img-md rounded-circle" alt="" src="${user.getAvatar() }"></td>
										<td>${user.getUsername() }</td>
										<td>${user.getFullname() }</td>
										<td>${user.getEmail() }</td>
										<td style="color: red">${userTop.getTotal() }</td>
									</tr>
								</php:if>
							</php:forEach>
						</php:forEach>
